...
/// The decoded video ground under the composited layer: that it is there, that it is the frame the
/// export would use, that it is decoded once per scrub, and that it refuses rather than vanishes.
mod underlay;

/// Which frame is still the one being asked for: generations, staleness and the overtaken claim.
mod generations;

/// What a composited frame costs at 1080p. Ignored by default: it measures, it does not assert.
mod cost;
...
fn the_published_frame_is_straight_alpha_and_the_composed_one_is_not() {
    ...
    // The transparent ground on purpose: the two alpha conventions differ only on a partly
    // transparent pixel, and a frame composed over an opaque video has none anywhere.
    let frame = host
...
fn renders_in_flight_are_bounded_and_the_bound_is_released_again() {
    ...
    assert_eq!(
        none.compose(&composition, 0, PreviewGround::Transparent)
            .err(),
...
fn a_frame_outside_the_converted_timeline_is_refused_without_a_render() {
    ...
    assert!(
        host.compose(&composition, frames - 1, PreviewGround::Transparent)
            .is_ok()
    );
    assert_eq!(
        host.compose(&composition, frames, PreviewGround::Transparent)
            .err(),
...
fn a_missing_device_is_its_own_refusal_and_retires_everything_rendered_on_it() {
    ...
    assert_eq!(
        lost.compose(&composition, 0, PreviewGround::Transparent)
            .err(),
