...
//!
//! [`osg_export::run_export`] needs one [`StagedText`]: a resolved face, **one** glyph atlas, and
//! **one run per cue** whose cells index that atlas. Two properties of the pieces that already exist
//! decide the shape of this boundary:
//!
...
//!   indexes the same atlas.
//!
//! So an export of *n* cues is one atlas whose cell table covers every cue, plus *n* runs. The atlas
//! crosses as it always has, through `glyph_atlas_stage`, and is addressed here by the handle that
//! command minted — never re-baked, never guessed at, never chosen by scanning the registry for
//! something that looks close. The runs cross here, as the positions the baker emitted, and are
//! copied rather than derived: this side accumulates no pen, wraps nothing and reorders nothing.
//!
//! # Why the handle is cross-checked
//!
...
//! to remove. The baker's own content hash is compared for the same reason the preview compares it.
...
/// version, so reading one out of an unknown payload is guessing.
pub(crate) const EXPORT_TEXT_SCHEMA_VERSION: u32 = 1;
...
pub(crate) struct ExportTextRequest {
    ...
    /// The handle `glyph_atlas_stage` minted for the baked atlas.
...
impl ExportTextRequest {
    ...
    /// not describe these cues, and [`refusal::atlas_unknown`] when no staged atlas answers to the
    /// handle it names.
    pub(crate) fn resolve(
        ...
        let atlas = atlases.descriptor(self.atlas_id, &self.atlas_content_hash)?;
        let runs = self.cues.into_iter().map(ExportCueRun::into_run).collect();
        Ok(StagedText::new(self.face, atlas, runs))
    }
}
