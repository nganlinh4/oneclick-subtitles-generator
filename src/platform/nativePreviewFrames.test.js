...
const frameResponse = (payload, overrides = {}) => ({
  // The URL addresses the only image in this published preview capability. Its transport index is
  // therefore zero even when `frameIndex` identifies a later scene instant.
  frameUrl: frameUrl(0),
...
describe('refusals', () => {
  ...
  it('refuses a response that does not identify the frame that was asked for', async () => {
    const cases = [
      ['wrong index', { frameIndex: 1 }, 'previewResponseFrameIndex'],
      ['wrong url index', { frameUrl: frameUrl(1) }, 'previewResponseFrameIndex'],
      ['unknown sequence', { sequenceId: '3f2504e0-4f89-41d3-9a0c-0305e82c3302' }, 'previewResponseSequence'],
      ['non v4 sequence', { frameUrl: frameUrl(0, '018f3a2b-1c4d-7e8f-9a0b-1c2d3e4f0001'), sequenceId: '018f3a2b-1c4d-7e8f-9a0b-1c2d3e4f0001' }, 'previewResponseSequence'],
      ['wrong size', { widthPx: 1_280 }, 'previewResponseSize'],
      ['unsupported mime type', { mimeType: 'image/svg+xml' }, 'previewResponseMimeType'],
      ['not a loopback frame', { frameUrl: 'https://cdn.example.com/frame/0.png' }, 'previewResponseUrl'],
      ['localhost alias', { frameUrl: frameUrl(0).replace('127.0.0.1', 'localhost') }, 'previewResponseUrl'],
      ['host suffix', { frameUrl: frameUrl(0).replace('127.0.0.1', '127.0.0.1.evil.example') }, 'previewResponseUrl'],
      ['missing credentials', { frameUrl: `http://127.0.0.1:49152/frame/${SEQUENCE_ID}/0` }, 'previewResponseUrl'],
      ['padded index', { frameUrl: frameUrl('00') }, 'previewResponseUrl'],
      ...
      ['wrong layer', { layer: 'subtitles' }, 'previewResponseLayer'],
      ['unknown layer', { layer: 'overlay' }, 'previewResponseLayer'],
      ['missing layer', { layer: undefined }, 'previewResponseLayer'],
      ['extra field', { extra: true }, 'previewResponseShape'],
    ];

    for (const [label, overrides, nativeCode] of cases) {
      acceptFrames(overrides);
      const error = await rejectionOf(createNativePreviewSurface().requestFrame(frameRequest(0)));
      expect(error.code, label).toBe('nativePreviewRejected');
      expect(error.nativeCode, label).toBe(nativeCode);
    }
  });
