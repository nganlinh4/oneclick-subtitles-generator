 ...
 *
 * `fallback` is what the surface shows when no native frame has decoded — during continuous
 * playback, before the first frame arrives, and whenever the compositor is unavailable for this
 * source. THE HANDOVER IS DECIDED HERE, in one place, on the only fact that matters: whether a real
 * frame is on screen. A caller deciding it from "a frame was returned" would hide the fallback
 * during the milliseconds the image is still decoding, and a subtitle that blinks out on pause is
 * precisely the kind of detail this migration is not allowed to introduce.
 *
...
const NativeCompositedFrame = ({
  ...
  const loading = visible && nextUrl !== null && nextUrl !== shown;
      ...
      {loading && (
        <img
          ...
          onLoad={() => {
            ...
            if (pendingRef.current === nextUrl) setShown(nextUrl);
          }}
          onError={() => {
