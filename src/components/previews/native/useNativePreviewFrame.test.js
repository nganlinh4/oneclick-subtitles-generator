...
const frameResponse = (payload) => ({
  // A preview capability publishes one transport frame at URL index zero; the response field below
  // independently identifies the requested scene instant.
  frameUrl: frameUrl(0),
...
describe('requesting the frame for the current instant', () => {
  it('asks for the frame index it was given and shows what comes back', async () => {
    ...
    expect(result.current.frame.url).toBe(frameUrl(0));
