...
// The guarded OSG router deliberately omits the upstream print route. Keep the
// upstream executor shape so the platform implementations remain a small,
// reviewable vendor delta, while making that intentional unreachability explicit.
#[allow(dead_code)]
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
// OSG removes navigation, direct-eval, print, and native-window mutation routes
// at the router. Their upstream executor methods remain only to minimize the
// security patch carried against the vendored MIT crate.
#[allow(dead_code)]
pub trait PlatformExecutor<R: Runtime>: Send + Sync {
    ...
    /// scrolling with nearest alignment only when its center is outside the viewport.
    /// Unlike [`Executor::get_element_rect`], this does not add scroll offsets: pointer
    /// events dispatch against viewport coordinates (`clientX`/`clientY`), so the center
    /// must be viewport-relative.
        ...
        let script = format!(
            r"(function() {{
                ...
                if (centerX < 0 || centerY < 0
                    ...
                    // Scrolling can synchronously run application handlers that replace the node.
                    // Resolve the stored WebDriver reference again before deriving pointer geometry.
                    el = window.{js_var};
                    if (!el || !el.isConnected) {{
                        throw new Error('stale element reference');
                    }}
        ...
        let script = format!(
            r"(function() {{
                ...
                if (el.tagName === 'INPUT' && el.type === 'file') {{
                    throw new Error('the guarded automation server refuses file-input activation');
                }}
                var r = el.getBoundingClientRect();
                var centerX = r.left + r.width / 2;
                var centerY = r.top + r.height / 2;
                if (centerX < 0 || centerY < 0
                    ...
                    // A scroll handler may synchronously rerender the control. Re-resolve the
                    // WebDriver reference and repeat the native-surface guard before clicking.
                    el = window.{js_var};
                    if (!el || !el.isConnected) {{
                        throw new Error('stale element reference');
                    }}
    ...
    async fn clear_element(&self, js_var: &str) -> Result<(), WebDriverErrorResponse> {
        let script = format!(
            r"(function() {{
                ...
                if (el.tagName === 'INPUT' && el.type === 'file') {{
                    throw new Error('the guarded automation server refuses file-input mutation');
                }}
                el.focus();
