...
async fn enforce_native_window_guard<R: Runtime + 'static>(
    State(state): State<Arc<AppState<R>>>,
    request: Request,
    next: Next,
) -> Result<Response, WebDriverErrorResponse> {
    state.enforce_window_guard()?;
...
pub fn create_router<R: Runtime + 'static>(state: Arc<AppState<R>>) -> Router {
    ...
    Router::new()
        ...
        // Deliberately no sessionless `/wdio/eval` endpoint. OSG uses the standard session-bound
        // W3C execute routes; an unauthenticated direct-eval endpoint would make the per-run
        // session identity meaningless.
        // =================================================================
        ...
        // Deliberately no print route: cross-platform WebDriver print implementations may open a
        // native print surface. The screenshot-backed OSG journeys do not use page printing.
        .layer(middleware::from_fn_with_state(
            guard_state,
            enforce_native_window_guard::<R>,
        ))
        .with_state(state)
}
